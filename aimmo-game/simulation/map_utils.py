...
class UpdateMapAndAvatars:

    world_map = None

    @staticmethod
    def update_avatars_and_map(world_map, no_of_avatars):
        """
        Update the world avatars and the map with the
        number of avatars.
        """
        UpdateMapAndAvatars.world_map = world_map
        UpdateMapAndAvatars._update_avatars()
        UpdateMapAndAvatars._update_map(no_of_avatars)
    ...
    def _add_outer_layer(self):
        ...
        * Add new pickups to compensate for more avatars.
        """
        self._add_vertical_layer(self.world_map.min_x() - 1)
        self._add_vertical_layer(self.world_map.max_x() + 1)
        self._add_horizontal_layer(self.world_map.min_y() - 1)
        self._add_horizontal_layer(self.world_map.max_y() + 1)

    def _add_vertical_layer(self, x):
        for y in range(self.world_map.min_y(), self.world_map.max_y() + 1):
            self.world_map.grid[Location(x, y)] = Cell(Location(x, y))
